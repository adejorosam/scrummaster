...
export class ScrumboardComponent implements OnInit {
  ...
  constructor(private router: Router,private dataservice: DataService,private dragula:DragulaService, private http : HttpClient) { 
    ...
    // this.dataservice.users = sessionStorage.getItem('data');
    ...
    this.http.get('http://127.0.0.1:8000/samsonadejoroscrumy/api/scrumusers/',this.dataservice.httpOptions).subscribe(
    data => {
      console.log(data);
  ...
  editGoal(event)
  {
    ...
    {
      this.http.put('http://' + this.dataservice.domain_name + '/samsonadejoroscrumy/api/goals/', JSON.stringify({'mode': 1, 'goal_id': items[0], 'new_name': goal_name}), this.dataservice.authOptions).subscribe(
      data => {
